...
npx create-agentic-boiler@latest my-app -y -p pnpm

# Skip install and git init for faster scaffolding
npx create-agentic-boiler@latest my-app -y -p npm --skip-install --skip-git
```
...
1. **Update package.json**: Set your author, repository URL, and version in `create-agentic-boiler/package.json`
2. **Test locally** (optional): Test the package before publishing:
   ```bash
   cd create-agentic-boiler
   npm link
   cd /path/to/test/directory
   create-agentic-boiler my-test-app
   ```
...
Edit any files in the main project (outside of `create-agentic-boiler/` folder):
...
#### Step 2: Navigate to create-agentic-boiler Directory
...
**What this does:**
- Deletes everything in `create-agentic-boiler/template/`
- Copies ALL files from your main project into `template/`
- Excludes: `node_modules`, `.next`, `.git`, lock files, `.env`, and the `create-agentic-boiler` folder itself
- Cleans up the template's `package.json` (removes `private` field and `sync-template` script)

#### Step 4: Bump the Version

Update the version number in `create-agentic-boiler/package.json`:
...
**Option B - Manual:**
Edit `create-agentic-boiler/package.json` and change the version number.
...
```bash
cd /path/to/test-directory
npx create-agentic-boiler@latest test-project
# or use the specific version
npx create-agentic-boiler@1.1.4 test-project
```
...
- Users may have cached versions. Tell them to:
  ```bash
  npx clear-npx-cache
  npx create-agentic-boiler@latest my-project
  # or use specific version
  npx create-agentic-boiler@1.1.4 my-project
  ```
